...
置き換えた（script.js）。旧 Canvas 2D 実装（Opus 4.7 作）は比較用に `opus/` に
無編集のまま保存してある。
...
- 手持ちカメラ風の微小な揺れ（prefers-reduced-motion ではカメラ揺れ無効 +
  初期風速0。風スライダーの明示操作によるアニメーションは尊重する）
- ジェスチャ: タップ=波紋状の突風が広がる / スワイプ=なでた方向に風向きが変わり
  突風が吹く（前線の移動も積分で連続） / 長押し=葉がかき分けられて指の下に
  光のプールが開く（ドラッグで追従、離すと閉じる）
- パラメータパネルは「調整」ボタンで開閉（Esc でも閉じる）
- 視点の俯角スライダー 35–90°（yaw→pitch の順で基底を構成し、真上 90° でも縮退しない。
  俯角が深いほどカメラ高を 1.5→3.0m に上げて視野の地面スパンを保つ）
- 呼吸・隙間変化の位相は CPU で積分して uniform 渡し
  （t×風係数 だと風を変えた瞬間に位相が飛び、スポットが瞬時に別の形になる）
